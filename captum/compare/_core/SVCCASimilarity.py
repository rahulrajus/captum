...
class SVCCASimilarity(ModuleSimilarity):
    r"""
    SVCCA similarity computes similarity metric by preprocessing activations using an SVD before running CCA. Includes several customization utilities for more granular access to how the comparison is done across different layer types.

    Paper- "SVCCA: Singular Vector Canonical Correlation Analysis for Deep Learning Dynamics and Interpretability". Neural Information Processing Systems (NeurIPS) 2017
    """
    ...
    def __init__(
        ...
        r"""
        Args:
            model1 (nn.Module): First model that will be compared
            model2 (nn.Module): Second model that will be compared
            model1_id (str): Unique identifier to use when saving/loading model1 activations
            model2_id (str): Unique identifier to use when saving/loading model2 activations
        """
    ...
    def compare(
        ...
        r"""
        Function for running SVCCA algorithm on models. Provides utilities for layer activation transforms, algorithm parameters, and methods for comparing one layer pair or multiple layer pairs. Function can also work on standalone layers in the case where models are not defined.
